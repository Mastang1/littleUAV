...
int8_t MpuInit(void) //初始化
{
	...
	date += IIC_Write_One_Byte(MPU6050_ADDRESS, GYRO_CONFIG, 0x10);//+-2000deg/s
	...
	if(date!= MPU6050_PRODUCT_ID&&date!= 0x98) //如果地址正确
		return FAILED;
	else 
		MpuGetOffset(); //调用校准数据
		return SUCCESS;
...
#define  Gyro_Read() IIC_read_Bytes(MPU6050_ADDRESS, 0X3B,buffer,6) //读取角速度
#define  Acc_Read() IIC_read_Bytes(MPU6050_ADDRESS, 0x43,&buffer[6],6)  //读取加速度
